...
int DoGotHook(const char* TargetDir,const char* TargetSoName,void* symbol,void* new_function,void** old_function){
    ...
    printf("Got size:%d\n",uiGotTabSize);
    uint32_t base=get_module_base(TargetPid,TargetSoName);
    ...
    if(bHaveFoundTargetAddr==1){
        printf("dogothook success\n");
    }
    else{
        printf("dogothook fail\n");
    }
    fclose(fp);
    return 0;
...
int main(int argc,char* argv[]){
    ...
    //char* path="/data/app/com.fancy.resecond-1/lib/arm/";
   // char* soname="libencode1.so";
    ...
    // printf("puts point:0x%x\n",&puts);
    // printf("myputs addr:%x\n",&myputs);
    int (*f)(const char*);
    DoGotHook(path,soname,(void*)&puts,(void*)&myputs,&oldfunc);
    printf("after hook puts addr:0x%x\n",puts);
    printf("after hook puts point:0x%x\n",&puts);
    f=&puts;
    (*f)("just 2\n");
    puts("just 3\n");
    test();
